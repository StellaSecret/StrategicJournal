...
data class EntryPrediction(val entry: JournalEntry, val prediction: Prediction)
data class EntryDecision(val entry: JournalEntry, val decision: Decision)
...
class ReviewViewModel @Inject constructor(
    ...
    private fun buildState(entries: List<JournalEntry>): ReviewUiState {
        ...
        val predictionAccuracy = if (reviewed.isNotEmpty())
            reviewed.count { it.wasCorrect == true }.toFloat() / reviewed.size else 0f

        val reviewedDec = reviewedDecisions.map { it.decision }
        val wouldRepeatRate = if (reviewedDec.isNotEmpty())
            reviewedDec.count { it.wouldRepeat == true }.toFloat() / reviewedDec.size else 0f

        val ratedDec = reviewedDec.filter { it.outcomeRating != null }
        val avgOutcomeRating = if (ratedDec.isNotEmpty())
            ratedDec.sumOf { it.outcomeRating!!.toDouble() }.toFloat() / ratedDec.size else 0f

        val avgConf = if (reviewed.isNotEmpty())
    ...
    fun reviewPrediction(entryId: String, predictionId: String, wasCorrect: Boolean, actualOutcome: String) {
        viewModelScope.launch {
            val entry = repository.getEntry(entryId) ?: return@launch
            val updated = entry.copy(
                updatedAt = LocalDateTime.now().toString(),
                predictions = entry.predictions.map { p ->
                    if (p.id == predictionId) p.copy(
