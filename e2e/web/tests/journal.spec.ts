...
test.describe('Home page', () => {
  ...
  test('"+ New entry" button navigates to /entry', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: 'Save entry' })).toBeVisible({ timeout: 10_000 })
  })
})
...
test.describe('Entry page', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/entry')
    // EntryPage renders null while the store loads, showing "Loading…".
    // Wait for Loading to disappear before asserting anything else.
    await expect(page.getByText('Loading…')).not.toBeVisible({ timeout: 10_000 })
    await expect(page.getByRole('button', { name: 'Save entry' })).toBeVisible({ timeout: 10_000 })
  })
